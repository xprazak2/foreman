  ...
  context 'using nested objects' do
    ...
    context 'resouce scoping' do
      ...
      it 'should return nested resource' do
        get :nested_values, params: { :foo_id => @foo_1.id, :id => @testable_1.id }
        assert_equal @testable_1, @controller.instance_variable_get('@testable')
        assert_equal @foo_1, @controller.instance_variable_get('@nested_obj')
      end
    end
