...
class Api::V2::SettingsControllerTest < ActionController::TestCase
  ...
  test "should update setting as canned admin" do
    user = user_one_as_canned_admin
    setting_id = Setting.where(:settings_type => 'integer').first.id
    as_user user do
      put :update, params: { :id => setting_id, :setting => { :value => "100" } }
    end
  ...
  test "should view setting as canned admin" do
    user = user_one_as_canned_admin
  ...
  def user_one_as_canned_admin
    user = users(:one)
    user.roles = [Role.default, Role.find_by_name('Canned admin')]
    user
  end
end
