...
class Api::V2::UsersControllerTest < ActionController::TestCase
  ...
  test "should create user with escalated roles as canned admin" do
    roles = [Role.find_by_name('Manager')]
    org = FactoryBot.create(:organization)
    # loc = FactoryBot.create(:location)
    user = FactoryBot.create :user, :login => 'ca',
                                    :role_ids => [roles(:canned_admin).id],
  ...
  # test "should create user with assigned organizations as canned admin" do
  #   organizations = [FactoryBot.create(:organization)]
  #   user = FactoryBot.create :user, :role_ids => [roles(:canned_admin).id]
  #   as_user user do
  #     post :create, params: { :user => min_valid_attrs.clone.update(:organization_ids => organizations.map { |org| org.id }) }
  #     assert_response :success
  #   end
  # end
end
