...
class RoleLockTest < ActiveSupport::TestCase
  ...
  test "should rename and create role" do
    original = roles(:create_hosts)
    original_id = original.id
    original_name = original.name
    new_name = "Customized #{original_name}"
    refute find_role(new_name)
    @role_lock.rename_and_create original, original_name, @permissions
    renamed_original = find_role(new_name)
    assert renamed_original
    assert_equal original_id, renamed_original.id
    new_role = find_role(original_name)
    assert new_role
    assert_equal @permissions, new_role.permissions.pluck(:name).map(&:to_sym)
    refute renamed_original.locked?
    assert_equal @role_lock.plugin_id, new_role.origin
  end

  test "should process existing role with changed permissions list" do
    role = roles(:create_hosts)
    original_name = role.name
    original_id = role.id
